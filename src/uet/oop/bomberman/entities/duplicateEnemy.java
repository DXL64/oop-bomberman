...
public class duplicateEnemy extends Enemy {
    ...
    public Image chooseSprite() {
        if (death) {
            if (count < 30) {       
                return Sprite.minvo_dead.getFxImage();
            }
            return null;
        }
        spriteImage = count % 9;
        if (direction == DIRECTION.LEFT || direction == DIRECTION.UP) {
            switch (spriteImage / 3) {
                case 0:
                    return Sprite.minvo_left1.getFxImage();
                case 1:
                    return Sprite.minvo_left2.getFxImage();
                case 2:
                    return Sprite.minvo_left3.getFxImage();
            }
        }
        else if (direction == DIRECTION.RIGHT || direction == DIRECTION.DOWN) {
            switch (spriteImage / 3) {
                case 0:
                    return Sprite.minvo_right1.getFxImage();
                case 1:
                    return Sprite.minvo_right2.getFxImage();
                case 2:
                    return Sprite.minvo_right3.getFxImage();
            }
        }
        return null;
    }
